...
class LmsPublicKey(object):
    ...
    def verify(self, message, sig):
        ...
        while node_num > 1:
            if (node_num % 2):
                 tmp = sha256_hash(self.I + pathvalue.next() + tmp + u32str(node_num / 2) + D_INTR)
            else:
                 tmp = sha256_hash(self.I + tmp + pathvalue.next() + u32str(node_num / 2) + D_INTR)
            node_num = node_num/2
        if (tmp == self.value):
            return VALID
        else:
    ...
    def parse(cls, buffer):
        lms_type = hex_u32_to_int(buffer[0:4])
    ...
    def deserialize(cls, buffer):
        ...
        if lms_type in lms_params:
            ...
            raise ValueError(err_unknown_typecode)
        lmots_type = hex_u32_to_int(buffer[4:8])
